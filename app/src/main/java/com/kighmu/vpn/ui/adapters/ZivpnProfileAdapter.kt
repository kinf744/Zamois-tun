...
class ZivpnProfileAdapter(
    private var profiles: MutableList<ZivpnProfile>,
    private val onSelectionChanged: (String, Boolean) -> Unit,
    private val onEdit: (ZivpnProfile) -> Unit,
    private val onDelete: (ZivpnProfile) -> Unit,
    private val onClone: (ZivpnProfile) -> Unit
) : RecyclerView.Adapter<ZivpnProfileAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.tvName.text = if (p.profileName.isNotEmpty()) p.profileName else "ZIVPN ${position + 1}"
        holder.tvSub.text = p.serverAddress
    ...
    fun setProfiles(newList: List<ZivpnProfile>) {
