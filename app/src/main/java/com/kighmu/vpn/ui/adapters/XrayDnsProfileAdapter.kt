...
class XrayDnsProfileAdapter(
    private val profiles: MutableList<XrayDnsProfile>,
    private val onSelectionChanged: (String, Boolean) -> Unit,
    private val onEdit: (XrayDnsProfile) -> Unit,
    private val onDelete: (XrayDnsProfile) -> Unit,
    private val onClone: (XrayDnsProfile) -> Unit
) : RecyclerView.Adapter<XrayDnsProfileAdapter.VH>() {
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.tvName.text = if (p.profileName.isNotEmpty()) p.profileName else "XrayDNS ${position + 1}"
    ...
    fun setProfiles(list: List<XrayDnsProfile>) {
        profiles.clear()
