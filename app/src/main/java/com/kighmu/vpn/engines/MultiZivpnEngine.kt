...
class MultiZivpnEngine(
    ...
    override suspend fun start(): Int {
        ...
        socksBalancer?.stop(); socksBalancer = null
        delay(500)
        ...
        selected.forEachIndexed { idx, profile ->
            ...
            val cfg = baseConfig.copy(
                zivpnHost = profile.serverAddress, zivpnPort = profile.serverPort,
                zivpnPassword = profile.password, zivpnObfs = profile.obfs)
            var port = -1; var attempt = 0
            ...
            else KighmuLogger.error(TAG, "Profil[${idx+1}] ECHEC definitif apres $MAX_RETRIES tentatives")
        }
        KighmuLogger.info(TAG, "=== STEP 2: ${successPorts.size}/${selected.size} profils ZIVPN connectes ===")
        if (successPorts.isEmpty()) throw Exception("Aucun profil ZIVPN connecte apres $MAX_RETRIES tentatives chacun")
        ...
        if (successPorts.size > 1) {
            ...
            balancer.start(); socksBalancer = balancer
    ...
    override fun startTun2Socks(fd: Int) {
        ...
        HevTun2Socks.init()
        val svc = vpnService ?: run { KighmuLogger.error(TAG, "VpnService null"); return }
        ...
        } else { KighmuLogger.error(TAG, "HevTun2Socks non disponible") }
    }
    ...
    override suspend fun stop() {
        ...
        synchronized(engines) {
            engines.forEach { engine ->
                try { runBlocking { engine.stop() } } catch (_: Exception) {}
            }
            engines.clear()
        }
        try { Runtime.getRuntime().exec(arrayOf("sh", "-c",
            "killall -9 libuz_core.so 2>/dev/null; pkill -9 -f libuz_core 2>/dev/null")) } catch (_: Exception) {}
        scope.cancel()
